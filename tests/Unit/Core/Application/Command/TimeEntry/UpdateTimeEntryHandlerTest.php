<?php

declare(strict_types=1);

namespace App\Tests\Unit\Core\Application\Command\TimeEntry;

use App\Core\Application\Command\TimeEntry\UpdateTimeEntry\UpdateTimeEntryCommand;
use App\Core\Application\Command\TimeEntry\UpdateTimeEntry\UpdateTimeEntryHandler;
use App\Core\Domain\DTO\Security\SecurityToken;
use App\Core\Domain\Exception\Security\ForbiddenException;
use App\Core\Domain\Model\Repository\TimeEntryRepository;
use App\Core\Domain\Model\VO\User\UserId;
use App\Core\Domain\Service\Security\RoleBasedSecurityChecker;
use App\Core\Domain\Service\TimeEntry\UpdateTimeEntry\UpdateTimeEntryServiceInterface;
use App\Shared\Domain\Enum\SystemRole;
use App\Tests\Unit\Core\Domain\Mother\TimeEntry\TimeEntryIdMother;
use PHPUnit\Framework\TestCase;
use Symfony\Component\Uid\Uuid;

final class UpdateTimeEntryHandlerTest extends TestCase
{
    public function test_GivenEmployeeUpdatingOwnTimeEntry_WhenInvoke_ThenServiceIsCalled(): void
    {
        $authUserId = Uuid::v4()->toRfc4122();
        $timeEntryId = (string) TimeEntryIdMother::create();

        $repo = $this->createMock(TimeEntryRepository::class);
        $repo->method('findOwnerUserId')->willReturn(new UserId($authUserId));

        $service = $this->createMock(UpdateTimeEntryServiceInterface::class);
        $service->expects(self::once())->method('__invoke');

        $handler = new UpdateTimeEntryHandler($service, $repo, new RoleBasedSecurityChecker());
        $handler(new UpdateTimeEntryCommand(
            securityToken: new SecurityToken($authUserId, SystemRole::EMPLOYEE),
            id: $timeEntryId,
            date: '2026-05-15',
            hours: '4.00',
            description: 'updated',
        ));
    }

    public function test_GivenEmployeeUpdatingOtherEmployeesTimeEntry_WhenInvoke_ThenForbidden(): void
    {
        $authUserId = Uuid::v4()->toRfc4122();
        $otherUserId = Uuid::v4()->toRfc4122();
        $timeEntryId = (string) TimeEntryIdMother::create();

        $repo = $this->createMock(TimeEntryRepository::class);
        $repo->method('findOwnerUserId')->willReturn(new UserId($otherUserId));

        $service = $this->createMock(UpdateTimeEntryServiceInterface::class);
        $service->expects(self::never())->method('__invoke');

        $handler = new UpdateTimeEntryHandler($service, $repo, new RoleBasedSecurityChecker());

        $this->expectException(ForbiddenException::class);

        $handler(new UpdateTimeEntryCommand(
            securityToken: new SecurityToken($authUserId, SystemRole::EMPLOYEE),
            id: $timeEntryId,
            date: '2026-05-15',
            hours: '4.00',
            description: 'updated',
        ));
    }

    public function test_GivenAdminUpdatingAnyTimeEntry_WhenInvoke_ThenServiceIsCalled(): void
    {
        $repo = $this->createMock(TimeEntryRepository::class);
        $repo->method('findOwnerUserId')->willReturn(new UserId(Uuid::v4()->toRfc4122()));

        $service = $this->createMock(UpdateTimeEntryServiceInterface::class);
        $service->expects(self::once())->method('__invoke');

        $handler = new UpdateTimeEntryHandler($service, $repo, new RoleBasedSecurityChecker());
        $handler(new UpdateTimeEntryCommand(
            securityToken: new SecurityToken(Uuid::v4()->toRfc4122(), SystemRole::ADMIN),
            id: (string) TimeEntryIdMother::create(),
            date: '2026-05-15',
            hours: '4.00',
            description: 'updated',
        ));
    }
}
